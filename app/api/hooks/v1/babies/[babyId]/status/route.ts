...
async function handleGet(req: NextRequest, ctx: ApiKeyContext, routeContext: any) {
  ...
  const [lastFeed, lastDiaper, lastPotty, lastSleep, lastBath, lastMedicine, lastSupplement, lastPump, activeBreastFeed] = await Promise.all([
  ...
  const [todaysFeeds, diapers, pottyCatches, sleepLogs, bathCount, medicineCount, supplementCount] = await Promise.all([
